...
public class UserMenu extends AbstractInlineHandler {
    ...
    public boolean onCallbackQuery(AbsSender bot, Update update, CallbackQuery query) throws TelegramApiException {
        return super.onCallbackQuery(bot, update, query);
    }

    @Override
    public void onCancel(AbsSender bot, Update update, Message message) {
        super.onCancel(bot, update, message);
    }
    ...
    private boolean handleAddUser(InlineMessageEvent event) throws TelegramApiException{
        ...
        switch (userData.getState()){
            case 1:{
                    ...
                    BotUtil.sendMessage(bot, message, "Great, now give me your name", false, false, null);
                    }
            case 2:{
                    final String text = message.getText();
    ...
    private boolean handleEditUserName(InlineCallbackEvent event) throws TelegramApiException{
        ...
        final IInlineMessageEvent onInputMassega = evt ->
                {
                    ...
                    if(userData.getState() == 1){
                        ...
                        if (creator.getLevel() <= userToEdit.getLevel()){
                            BotUtil.sendMessage(evt.getBot(), evt.getMessage(), "You cannot edit user with higher power then yours, try again!",false, false, null);
                            return true;
                        }
        ...
        final List<DBUser> usersList = userService.findAll();
        final InlineUserData userData = event.getContext().getUserData(event.getQuery().getFrom().getId());
    ...
    private boolean handleEditAccessLevel(InlineCallbackEvent event) throws TelegramApiException{
        ...
        final InlineUserData userData = event.getContext().getUserData(event.getQuery().getFrom().getId());
        final InlineMenuBuilder usersBuilder = new InlineMenuBuilder(event.getContext(), userData.getActiveMenu());
        usersBuilder.name("Select users:");
